...
class MaskedConv2D(Conv2D):
    ...
    def prune_by_similarity(self):
        ...
        idx = self.name
        idx = int(idx[5:])
        if tensor.shape[0] == 1 and idx < 80:
            wait_for_prune = []
            ...
            threshold = np.max(similar[similar!=-2147483647]) * 0.95 #Bigger 0.9
            ...
            masking = np.ones_like(tensor)
            final_mask = np.where(mask == 0 or new_mask == 0, 0, masking)
            self.mask = tf.convert_to_tensor(final_mask)
            if biasFlag:
